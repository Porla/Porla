...
async function load(options) {
    ...
    // Load session
    await session.init(options.database, options);
    await emit('session.init', { session });
    await session.load(options.database, options);
...
function loadDefaultDatabase() {
    const p = path.join(process.cwd(), 'data', 'porla.sqlite');
...
function porla(options) {
    ...
    // Set default options
    options.database = options.database || loadDefaultDatabase();
    ...
    const shutdown = async () => {
        ...
        await session.unload(options.database);
